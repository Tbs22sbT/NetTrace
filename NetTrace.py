...
class AnalyzerWorker(QObject):
    ...
    def __init__(self, ips, batch_size=50, delay=1):
        super().__init__()
        self.ips = ips
        ...
        try:
            for idx, ip in enumerate(self.ips, start=1):
                if not is_valid_ip(ip):
                    results.append({'IP': ip, 'Error': 'IP no válida'})
                elif is_private_or_reserved_ip(ip):
                    results.append({'IP': ip, 'Error': 'IP privada o reservada'})
                else:
...
""")
    ...
    def on_analyze(self):
        ...
        if mode_idx == 0:  # Introducir IPs
            ...
            path, _ = QFileDialog.getOpenFileName(self, "Abrir Excel", filter="Excel Files (*.xlsx)")
            if not path:
    ...
    def on_finished(self, results):
        ...
        if self.output_combo.currentText() == "Excel":
            ...
            df = df[columns]
            path, _ = QFileDialog.getSaveFileName(self, "Guardar resultados", filter="Excel Files (*.xlsx)")
            if path:
